#!/usr/bin/env python3
import http.server, json, os, urllib.request, urllib.error, urllib.parse
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif p.startswith("/api/sync/"):
            # Load sync data for a store ID
            sid = p[len("/api/sync/"):]
            sid = urllib.parse.unquote(sid).replace("/","").replace("..","")[:50]
            fname = f"sync_{sid}.json"
    ...
    def do_POST(self):
        ...
        if self.path == "/api/key":
            ...
            try:
                ...
                # Try models in order — fallback if 503/429
                    ...
                    for attempt in range(2):  # 2 attempts per model
                        try:
                            ...
                            if code in (503, 429, 500):
                                import time; time.sleep(1)  # wait 1s then retry
                                continue
                            else:
                ...
                if result:
                    ...
                    self.send_header("Content-Type", "application/json")
                    self.cors(); self.end_headers()
                    self.wfile.write(last_err or b'{"error":"All models unavailable, please try again"}')
            ...
            try:
                ...
                for model in models:
                    try:
                        ...
                        last_err = e.read()
                        if e.code in (503, 429, 500): continue
                        else: raise
                ...
                if result:
                    ...
                    self.send_header("Content-Type", "application/json")
                    self.cors(); self.end_headers()
                    self.wfile.write(last_err or b'{"error":"All models unavailable"}')
            ...
            # Save sync data for a store ID
            sid = self.path[len("/api/sync/"):]
            sid = urllib.parse.unquote(sid).replace("/","").replace("..","")[:50]
            if not sid:
                self.json({"error":"Invalid store ID"}, 400); return
            try:
                data = json.loads(body)
                fname = f"sync_{sid}.json"
                with open(fname,"w") as f: json.dump(data, f)
                self.json({"ok": True, "id": sid})
                print(f"  ✅ Synced store: {sid} ({len(data.get('items',[]))} items)")
            except Exception as e:
                self.json({"error": str(e)}, 500)
